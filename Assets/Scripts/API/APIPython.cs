    ...
    namespace API
    {
        public class APIPython : MonoBehaviour
        {
            ...
            public static UnityMolStructure load(string filePath, bool readHetm = true, bool forceDSSP = false, bool showDefaultRep = true, bool center = false, bool modelsAsTraj = true)
            {
                ...
                // Assets\Scripts\ParseData\Reader.cs
            ...
            public static void deleteStructure(UnityMolStructure s)
            {
            ...
            // !!!!!!!
            public static void cacheDelete(string structureName)
            {
                // Assets\Scripts\UnityMolMolecule\UnityMolStructureManager.cs
                UnityMolStructureManager sm = UnityMolMain.getStructureManager();
                sm.DeleteAll();
                UnityMolStructure s = sm.GetStructure(structureName);
                // Assets\Scripts\UnityMolMolecule\UnityMolSelectionManager.cs
                // UnityMolSelectionManager slm = UnityMolMain.getSelectionManager();
                // string selName = "selection";
                // UnityMolSelection sel = slm.selections[selName];
                if (s != null)
                { //Remove a complete loaded molecule
                    sm.Delete(s);
                    Debug.LogWarning("Deleting molecule '" + structureName + "'");
            ...
            // !!!
            public static void deleteShuang(UnityMolStructure s,UnityMolStructureManager sm)
            {
            ...
            public static void saveToPDB(string selName, string filePath, bool writeSSinfo = false)
            {
                ...
                if (selM.selections.ContainsKey(selName))
                {
                    //Debug.LogError(" let me see see what is selName:" + selName);
                    UnityMolSelection sel = selM.selections[selName];
                    //Debug.LogError(" let me see see what is sel:" + selName);
                    //全是Object
                    if (sel.Count == 0)
                    {
            ...
            public static String getAnimoName(){
                string selName = "selection";
                UnityMolSelectionManager selM = UnityMolMain.getSelectionManager();
                ...
                if (selM.selections.ContainsKey(selName))
                {
                    ...
                    // Assets\Scripts\ParseData\PDBReader.cs
                    String animoName = PDBReader.getAnimoName(sel);
                    return animoName;
            ...
            //为了避免玩炸了，不干扰其他的程序，所以重新写了一个，其本质功能大体与savetoPDB类似
            public static void cacheSave(string selName, string filePath, bool writeSSinfo = false)
            {
                UnityMolSelectionManager selM = UnityMolMain.getSelectionManager();
                ...
                if (selM.selections.ContainsKey(selName))
                {
                    ...
                    {
                        atomPos[id++] = strucPar.InverseTransformPoint(a.curWorldPosition);
                        // Debug.Log("!!!!atoms "+ atomPos[id]+"   id: "+id);
                    }
                    // Assets\Scripts\ParseData\PDBReader.cs
                    Debug.Log("!!!!let me see see sel"+ sel.ToString());
                    Debug.Log("!!!!let me see see atomPos"+ atomPos.ToString());
                    Debug.Log("!!!!let me see see writeSSinfo"+ writeSSinfo.ToString());


                    string pdbLines = PDBReader.ConnectWrite(sel, overridedPos: atomPos, writeSS: writeSSinfo);
                    UnityMolStructureManager sm = UnityMolMain.getStructureManager();
                    sm.DeleteAll();
                    // UnityMolMain.recordUndoPythonCommand("deleteSelection(\"" + sel.name + "\")");
                    // selM.deleteSelection();
                    // string stName = st.name;
                    
                    // APIPython.refreshStructure(APIPython.CacheRemoveAtom(selM));
                    // 这个地方已经变成两个了？
                    Debug.Log("!!!!let me see see pdblines"+ pdbLines);
                    //输出
                    try
                    {
                        StreamWriter writer = new StreamWriter(filePath, false);
            ...
            public static void cacheSaveWithoutMove(string selName, string filePath, bool writeSSinfo = false)
            {
                UnityMolSelectionManager selM = UnityMolMain.getSelectionManager();
            ...
            public static void saveSelectionToPDB(string path = null)
            {
                ...
                //E:\shuang\t4
                Debug.LogWarning("!!!selName:  "+selName);
                // selection
                saveToPDB(selName, path, false);
            }
            //快速导入
            public static void quickLoad(){
                string readPath = "E:/shuang/quickCache.pdb";
                APIPython.load(readPath,true,false,true,false);
            }
            //快速存储
            public static void quickSave(){
                string path = "E:\\shuang\\quickCache.pdb";
                string selName = "selection";
                saveToPDB(selName,path,false);
            }
            //结构复制
            public static void copyStructure(){
                string path = "E:\\shuang\\copyCache.pdb";
                string readPath = "E:/shuang/copyCache.pdb";
                string selName = "selection";
                saveToPDB(selName,path,false);
                APIPython.load(readPath,true,false,true,false);
            }
            ...
            // connection 控制
            public static void connectAnimoSave()
            {
                string path = "E:\\shuang\\cache.pdb";
                string readPath = "E:/shuang/cache.pdb";
                string selName = "selection";
                cacheSave(selName,path,false);
                // Vector3 pos = Vector3.zero, rot = Vector3.zero;
                // APIPython.getStructurePositionRotation(selName, ref pos, ref rot);
                // APIPython.delete(stName);
                // APIPython.load(path + String.Format("\\{0}.pdb", stName));
                APIPython.load(readPath,true,false,true,true);
                // Debug.Log("!!!!path:"+path + String.Format("\\{0}.pdb", stName));
                // APIPython.setStructurePositionRotation(stName, pos, rot);
                // 
                APIPython.delete(selName);
                // refreshStructure(null);
                // APIPython.setStructurePositionRotation(selName, 0, 0);
            }
            public static void connectAnimoWithoutMove(){
                UnityMolSelectionManager slm = UnityMolMain.getSelectionManager();
                UnityMolStructure st = slm.getCurrentSelection().structures[0];
                UnityMolStructure st2 = slm.getCurrentSelection().structures[1];
                UnityMolSelectionManager selM = UnityMolMain.getSelectionManager();
                string stName = st.name;
                // string path = Application.temporaryCachePath;
                string path = "E:\\shuang\\cache.pdb";
                // args = String.Format("-ipdb \"{0}\\{1}_in.pdb\" -opdb -O \"{0}\\{1}.pdb\"", path, stName) + args;
                
                string selName = getCurrentSelectionName();
                // APIPython.saveToPDB(selName, path + String.Format("\\{0}_in.pdb",stName));
                cacheSaveWithoutMove(selName,path ,false);
            ...
            // !!!!
            public static void DelSelect(){
                UnityMolSelectionManager slm = UnityMolMain.getSelectionManager();
                // UnityMolSelection sel = selM.selections[selName]
                // string selName = "selection";
                int structCount = slm.getCurrentSelection().structures.Count;
                List<UnityMolStructure> sts = slm.getCurrentSelection().structures;
                // for(int i = 0;i<structCount;i++){
                //     // UnityMolSelection st = slm.selections[selName];
                //     UnityMolStructure sts[i] = slm.getCurrentSelection().structures[i];
                // }
                for(int i = 0;i<structCount;i++){
                    deleteStructure(sts[i]);
                }
            }
            private static void CacheStartOBProcess()
            {
                // string args = "-h";
                UnityMolSelectionManager slm = UnityMolMain.getSelectionManager();
                // UnityMolSelection sel = selM.selections[selName]
                string selName = "selection";
                for(int i = 0;i<slm.getCurrentSelection().structures.Count;i++){
                    // UnityMolSelection st = slm.selections[selName];
                    UnityMolStructure st = slm.getCurrentSelection().structures[i];
                    Debug.Log("!!!!!!!delete:"+i);
                    string stName = st.name;
                    APIPython.cacheDelete(stName);
                }
            ...
            // !!!!!
            private static void TestShuang(string args)
            {
                UnityMolSelectionManager slm = UnityMolMain.getSelectionManager();
                UnityMolStructure st = slm.getCurrentSelection().structures[0];
                UnityMolStructure stT = slm.getCurrentSelection().structures[1];
                for(int i=0;i<slm.getCurrentSelection().structures.Count;i++){
                    Debug.LogWarning("!!!!!!'" + slm.getCurrentSelection().structures[i].name + "!!!!!");
                }
                // for(UnityMolStructure testSt:slm.getCurrentSelection().structures){
                //     Debug.LogWarning("!!!!!!'" + testSt + "!!!!!");
                // }
                string stName = st.name;
                string stTName = stT.name;
                Debug.LogWarning("!!EE!!!!'" + stName  + "!!!!!");
                Debug.LogWarning("!!!EE!!!'" + stTName  + "!!!EE!!");
                string path = Application.temporaryCachePath;
                args = String.Format("-ipdb \"{0}\\{1}_in.pdb\" -opdb -O \"{0}\\{1}.pdb\"", path, stName) + args;
                ...
                APIPython.getStructurePositionRotation(stName, ref pos, ref rot);
                APIPython.delete(stName);
                // APIPython.delete(stTName);
                // APIPython.load(path + String.Format("\\{0}.pdb", selName));
                APIPython.load(path + String.Format("\\{0}.pdb", stName));
                APIPython.setStructurePositionRotation(stName, pos, rot);
                // APIPython.setStructurePositionRotation(selName, pos, rot);
                // APIPython.deleteShuang(stT,slm);
                // APIPython.delete(stTName);
            }
            // !!!!!
            // public static void Minimization(int steps)
            // {
            //     String args = String.Format(" --minimize --step {0} -d",steps);
            //     StartOBProcess(args);
            // }
            // !!!!!
            public static void Minimization(int steps)
            ...
            //         return null;
            //     }
            //     UnityMolAtom atom1=sl.atoms[0], atom2 = sl.atoms[1];
            //     st.models[0].bonds.Add(atom1, atom2);
            //     //float dis = Vector3.Distance(atom1.position, atom2.position);
            //     //float threshhold = 1.5F;
            //     //Vector3 move2;
            //     //if (dis > threshhold && !isConnected(sl))
            //     //{
            //     //    move2 = atom1.position - atom2.position;
            //     //    move2 = move2.normalized * (move2.magnitude - threshhold);
            //     //}
            //     //HashSet<UnityMolAtom> v = new HashSet<UnityMolAtom>();
            //     //v.Add(atom2);

            //     //void s(UnityMolAtom a)
            //     //{
            //     //    UnityMolAtom[] res;
            //     //    if(st.models[0].bonds.bondsDual.TryGetValue(a,out res))
            //     //    {
            //     //        foreach(UnityMolAtom a2 in res)
            //     //        {
            //     //            s(a2);
            //     //        }
            //     //    }
            ...
            // !!!cache remove
            public static string CacheRemoveAtom(UnityMolSelectionManager sm)
            {
                UnityMolSelection sl = null;
                if (sl == null)
                    sl = sm.getCurrentSelection();
                if (sl.structures.Count > 1)
                    return null;
