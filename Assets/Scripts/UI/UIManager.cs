...
namespace UMol
{
    public class UIManager : MonoBehaviour
    {
        ...
        // cacheload自我更改
        public void cacheLoad(bool forceDesktop = false)
        {
            ...
            {
                rsfwb = gameObject.AddComponent<ReadSaveFilesWithBrowser>();
                Debug.Log("!!!!!read!!null"+rsfwb.ToString());
            }
            // 搞错了，这玩意儿只是打开面板，打开面板过后点击的那个load才是最终的记载的程序
            Debug.Log("!!!!!read"+rsfwb.ToString());
            // Assets\Scripts\Utilities\ReadSaveFilesWithBrowser.cs
            rsfwb.readFiles(readHTM, forceDesktop);
        }
        // 氨基酸文件调用和导入，
        public void loadAmino(string name)
        {
            string path=Application.dataPath+"/Resources/pdb/amino acids/"+name+"_ideal.pdb";
            // Debug.LogWarning("name:" + name + "!");
             API.APIPython.load(path,true,false,true,false);
            //  API.APIPython.load(path,true,false,true,true);
        }
        public void rename(){
            GameObject go = GameObject.Find("ButtonShowName");
            Text[] text = go.GetComponentsInChildren<Text>();
            String s = APIPython.getAnimoName();
            text[0].text = s;
        ...
        // 新增Connection
        public void connectAnimo()
        {
            APIPython.connectAnimoSave();
        }
        //结构复制
        public void copyStructure(){
            APIPython.copyStructure();
        }
        //快速存储
        ...
        public void connectAnimoWithoutMove(){
            APIPython.connectAnimoWithoutMove();
        }
